...
{
    start_tick = 100000;		// (r11921111 edit)
    burst_time = 100000; 		// (r11921111 edit)
    priority = 100000;			// (r11921111 edit)
    name = threadName;
    stackTop = NULL;
    stack = NULL;
    status = JUST_CREATED;
...
void testfunc(){	
	...
	printf("%s remain time : %d\n", t->getName(), t->burst_time);	
...
Thread::SelfTest1()
{
	...
	char *thread_name[thread_num] = {"A", "B", "C", "D", "E"};
	int thread_burst[thread_num] = {5, 3, 4, 1, 2};
	int thread_priority[thread_num] = {3, 1, 4, 5, 2};
	Thread *t;
	for (int i = 0; i < thread_num; i ++){
		t = new Thread(thread_name[i]);
		t->start_tick = kernel->stats->totalTicks;
		t->burst_time = thread_burst[i];
		t->priority = thread_priority[i];
		printf("Create thread %s !\n", thread_name[i]);
		t->Fork((VoidFunctionPtr) testfunc, (void *) NULL);
		kernel->scheduler->Print();
	}
	kernel->currentThread->Yield();    
}
...
{	
	...
	const int thread_num = 5;
	char *thread_name[thread_num] = {"A", "B", "C", "D", "E"};
	int thread_burst[thread_num] = {3, 5, 2, 4, 1};
	int thread_priority[thread_num] = {2, 4, 3, 5, 1};
	Thread *t;
