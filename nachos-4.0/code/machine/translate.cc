...
// from the virtual address
	...
	if (vpn >= pageTableSize) {
		...
		int frameNum = 0;
		while(frameNum < NumPhysPages && kernel->machine->FrameTable[frameNum].valid == TRUE) frameNum ++;
		if(frameNum < NumPhysPages){
			// Swap in 
			kernel->machine->FrameTable[frameNum].valid = TRUE;
			kernel->machine->FrameTable[frameNum].addrSpace = kernel->currentThread->space;
			kernel->machine->FrameTable[frameNum].vpn = vpn;
			kernel->machine->FrameTable[frameNum].inTime = kernel->stats->totalTicks;

			pageTable[vpn].physicalPage = frameNum;
			pageTable[vpn].valid = TRUE;
			
			kernel->swap->ReadSector(
				pageTable[vpn].virtualPage,
				&(kernel->machine->mainMemory[PageSize * frameNum]));

		}else{
			// Swap out
			int victim = 0, minTime = kernel->machine->FrameTable[0].inTime;
			for(int j = 0; j < NumPhysPages; j ++){
				if(kernel->machine->FrameTable[j].inTime < minTime){
					victim = j;
			...
			//Swap in
			kernel->machine->FrameTable[victim].valid = TRUE;
			kernel->machine->FrameTable[victim].addrSpace = kernel->currentThread->space;
			kernel->machine->FrameTable[victim].vpn = vpn;
			kernel->machine->FrameTable[victim].inTime = kernel->stats->totalTicks;
